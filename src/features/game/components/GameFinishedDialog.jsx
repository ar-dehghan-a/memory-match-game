  ...
  return (
    <Dialog open={open} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader>
          <DialogTitle
            className={cn('text-center text-2xl', isWin && 'text-primary', isGameOver && 'text-destructive')}
          >
          ...
          {isWin && currentScore && (
            <div className="mt-3 space-y-1 text-center text-sm text-muted-foreground">
              <p>
                عملکرد این دست:{' '}
                <span className="font-medium text-foreground">{toPersian(currentScore.movesUsed)}</span> حرکت،
                در زمان{' '}
              ...
              {bestScore && (
                <p>
                  بهترین رکورد:{' '}
                  <span className="font-medium text-foreground">{toPersian(bestScore.movesUsed)}</span> حرکت،{' '}
                  در زمان{' '}
